...
void ofApp::createRadialLines(int count) {
	...
	for (int i = 0; i < count; i++) {
		...
		// Create the nodes along the radial line
		for (int j = 0; j < lineLength; j++) {
			float dist = j * spacing;
			ofVec3f dir = ofVec3f(cos(angle), sin(angle), 0); // Direction vector for this line
			ofVec3f basePos = dir * dist; // Base position for this node

			// Each node has a unique time offset to create independent oscillations
			float timeOffset = (float)j / lineLength * TWO_PI;
			line.push_back(SphereNode(basePos, amplitude, frequency, timeOffset));
		}
...
void ofApp::draw() {
	...
	for (int i = 0; i < radialLines.size(); i++) {
		...
		// Draw each node in the radial line
		for (int j = 0; j < radialLines[i].size(); j++) {
			radialLines[i][j].draw(time); // Draw each node oscillating
		}
	}
...
void ofApp::addNodeToAllLines() {
	for (int i = 0; i < radialLines.size(); i++) {
		...
		// Keep same amplitude and frequency for all lines
		float timeOffset = (float)j / radialLines[i].size() * TWO_PI;
		radialLines[i].push_back(SphereNode(basePos, amplitude, frequency, timeOffset));
	}
}
