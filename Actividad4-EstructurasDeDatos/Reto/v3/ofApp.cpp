...
void ofApp::createRadialLines(int count) {
	...
	for (int i = 0; i < count; i++) {
		...
		line.push_back(SphereNode(basePos, 0, 0, 0)); // Static node, no oscillation

		// Add subsequent nodes that will oscillate vertically and horizontally
		for (int j = 1; j < lineLength / spacing; j++) {
			dist = j * spacing;
			basePos = dir * dist;
			line.push_back(SphereNode(basePos, 0, amplitude, frequency));
		}
...
void ofApp::draw() {
	...
	for (int i = 0; i < radialLines.size(); i++) {
		...
		radialLines[i][0].draw(time, ofColor(255, 255, 255), 0); // Static first node

		// Draw the dynamic nodes (rope effect)
		for (int j = 1; j < radialLines[i].size(); j++) {
			radialLines[i][j].draw(time, color, i * (TWO_PI / radialLines.size())); // Sync lines with same sine pattern
		}
	}
...
void ofApp::addNodeToAllLines() {
	for (int i = 0; i < radialLines.size(); i++) {
		...
		radialLines[i].push_back(SphereNode(basePos, 0, amplitude, frequency));
	}
}
